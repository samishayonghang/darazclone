...
def send_activation_email(recipient_email,activation_url):
        ...
        html_content=render_to_string('daraz/activate.html',{'activation_url':activation_url})
...
def send_resetpassword_email(recipient_email,reset_url):
        subject="Reset your password " + settings.SITE_NAME
        from_email='noreply@demomailtrap.com'
        to_email=[recipient_email]
        html_content=render_to_string('daraz/resetemail.html',{'reset_url':reset_url})
        text_content=strip_tags(html_content)
        email=EmailMultiAlternatives(subject,text_content,from_email,to_email)
        email.attach_alternative(html_content,'text/html')
        SendEmailThread(email).start()
